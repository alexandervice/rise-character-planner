  ...
  return (
    <div className="flex bg-zinc-800 rounded flex-wrap justify-center py-5">
      ...
      {allRaces.map((race, index) => (
        <div key={index} className={`m-4  ${characterData && characterData.race === race ? 'border-4 border-blue-500' : ''}`} onClick={() => handleRaceSelection(race)} onMouseEnter={() => setHovered(race)} onMouseLeave={() => setHovered(null)}>
